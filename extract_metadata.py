...
def extract_tableau_metadata(twbx_path: str) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for root_dir, _, files in os.walk(tmpdir):
            for f in files:
                if f.endswith(".twb"):
                    twb_file = os.path.join(root_dir, f)
                    break
        ...
        # -----------------------------------------
        # COLUMN → TABLE RESOLUTION MAP (CRITICAL)
        # -----------------------------------------
        column_to_table = {}

        for meta in root.findall(".//metadata-record[@class='column']"):
            local = meta.findtext("local-name")
            parent = meta.findtext("parent-name")

            if local and parent:
                col = clean_name(local)
                tbl = clean_name(parent)
                column_to_table[col] = tbl

        # -----------------------------------------
        # MODEL TABLES (DEDUPLICATED)
        # -----------------------------------------
        table_map = {}

        for relation in root.findall(".//relation[@type='table']"):
            raw_table = relation.get("table") or relation.get("name")
            table_name = clean_name(raw_table)
            ...
            for col in relation.findall(".//column"):
                cname = clean_name(col.get("name"))
                if cname:
                    cols.append(cname)

            table_map[table_name] = sorted(set(cols))
        ...
        for join in root.findall(".//relation[@type='join']"):
            for clause in join.findall(".//clause"):
                ...
                def parse(expr):
                    txt = expr.text.replace("[", "").replace("]", "")
                    parts = txt.split(".")
                    return parts[0], parts[-1]

                l_table, l_col = parse(exprs[0])
                r_table, r_col = parse(exprs[1])
        ...
        # -----------------------------------------
        # WORKSHEETS (VISUALS — FORMAT UNCHANGED)
        # -----------------------------------------
        for worksheet in root.findall(".//worksheet"):
            bound_columns = []
            seen = set()
            ...
            for col in worksheet.findall(".//column-instance"):
                ...
                clean_col = clean_name(col_ref.split(".")[-1])
                table = column_to_table.get(clean_col, "UnknownTable")
            ...
            for mark in worksheet.findall(".//pane/mark"):
                cls = mark.get("class")
                if cls:
                    visual_type = MARK_MAP.get(cls.lower(), visual_type)
                    break
        ...
        for dashboard in root.findall(".//dashboard"):
            ...
            for zone in dashboard.findall(".//zone"):
                if zone.get("name"):
                    sheets.append(zone.get("name"))

            metadata["dashboards"].append({
                "dashboardName": dashboard.get("name"),
                "worksheets": list(set(sheets))
            })
...
def handle_extraction(payload: ExtractMetadataRequest):
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            base = unquote(os.path.basename(payload.inputBlobUrl))
            out_name = os.path.splitext(base)[0] + "_metadata.json"
